...
const ViewSubject = () => {
  const {isLoading, getSubjectById,singleSubject,updateSubject}=useSubjectContext();
  const {id}=useParams();
  ...
  const handleSubmit=async(e)=>{
    e.preventDefault();
    if(subjectData!==singleSubject){
      await updateSubject(subjectData,subjectData.id);
    }
  }
  ...
  return (
    <Wrapper>
      <form onSubmit={handleSubmit} className="form">
        <h1>Subject</h1>
        {Object.entries(singleSubject).map(([key,value])=>(
          <TextField
          key={key}
          name={key}
          className="input"
